...
class Abstract_ReKV:
    ...
    def _encode_video_chunk(self, video_chunk): #编码视频片段,只为更新kv_cache
        ...
        video_features = self._get_video_features(pixel_values_videos)  # (1, Nv*196, D) 每个frame有196个token
        assert self.n_local >= video_features.shape[1], f'n_local: {self.n_local}, video_features: {video_features.shape[1]}'

        output = self.language_model(inputs_embeds=video_features, past_key_values=self.kv_cache, use_cache=True, return_dict=True) #对应patch.py里的model_forward
        self.kv_cache = output.past_key_values
    ...
    def encode_video(self, video, encode_chunk_size=64):  # video: (Nv, H, W, 3)
        ...
        for chunk_idx in range(num_chunks):
            ...
            self._encode_video_chunk(chunk_video) #每一次_encode_video_chunk都会init attention layer个数的contextmanager
