...
class ContextManager:
    ...
    def _append(
        ...
        attn.append(
            local_h_q, local_h_k, local_h_v,  #local_h_k里有init
        ...
        attn.append(
            global_h_q, global_h_k, global_h_v,   #global_h_k里也有init，重复了 2种办法：1.init后把init从local里拿走 2.没有填满local的时候还是只算上面的attention
    ...
    def append(
        ...
        # append local KV
        #self.org_klen = self.local_k.size(-2)
        ...
        for st in range(0, input_length, self.exc_block_size):  # Process the input tokens in blocks. 每次处理exc_block_size
            ...
            #self.processed_chunk_len = (st + 1) * self.exc_block_size
