...
class VectorTensor:
    ...
    def get_window_cosine_similarity(self, window_size, tensor, segment_summary_sign):
        assert tensor.dim() == 1 and tensor.size(0) == self.hidden_size, f'{tensor.size(0)}, {self.hidden_size}'
        if segment_summary_sign == "True":
        ...
        assert logits.dim() == 1 and logits.size(0) == window_size-1 if segment_summary_sign=="True" else window_size ,f'{logits.dim()}, {logits.size(0)},{window_size}'
        return logits
...
class ContextManager:
    def __init__(self, 
        ...
        self.max_retrive_block = max_cached_block
    ...
    def init(
        ...
        buffer_len = self.max_retrive_block * self.block_size + self.n_init
    ...
    def calculate_score(self, video_len, prompt):
        ...
        logits = torch.stack([self.block_k[u].get_window_cosine_similarity(num_frames,prompt[u],self.segment_summary_sign) for u in range(self.num_units)]) 
    ...
    def compress_kv(self, video_len, prompt = None):
        ...
        if self.compress_mode == 'online':
            ...
            assert logits.shape == (self.num_units, num_frames - 1) if self.segment_summary_sign == "True" else (self.num_units, num_frames)
        ...
        if self.encode_mode == 'sparse':
            ...
            batch = kept_indices.size(0) 
            offsets = torch.arange(block_size, device=kept_indices.device) 
            token_indices = kept_indices.unsqueeze(-1) * block_size + offsets 
            token_indices = token_indices.reshape(batch, -1)
    ...
    def _calc_block_topk(
        ...
        if self.retrieve_local == 'False':
            ...
            if self.retrieve_mode == 'online' and self.retrieval_prefill:
