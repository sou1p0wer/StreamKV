...
class ContextManager:
    ...
    def init(
        ...
        # L = n_init + n_retrieve
        buffer_len = self.topk * self.block_size + self.n_init #token level
    ...
    def get_retrieved_kv(self, query=None): #将init KV和retrieved KV加载到global_h_k, global_h_v，没global_buffer什么事
        ...
        if query is not None:  # retrieve based on the attention score between query and context's representative keys
            block_topk = self._calc_block_topk(query) # block_topk: batch_size x topk
            self.set_retrieved_block_indices(block_topk)

        assert len(self.retrieved_block_indices) == self.num_units

        global_h_k = self.global_buffer[0] #长度是有上限的
        global_h_v = self.global_buffer[1] #长度是有上限的

        with torch.cuda.stream(GLOBAL_STREAM):
            if self.init_exc:  # init KV were loaded in global_h_k, context KV were offloaded in global_blocks 如果已经加载过init KV，则从global_blocks中reload并copy到global_h_k, global_h_v
                # offload LRU blocks
            ...
            else:  # init KV and context are in self.global_remainder 如果没有加载过init KV，则从self.global_remainder中加载init KV和context KV到global_h_k, global_h_v
                # load init KV
                init_st = 0
    ...
    def get_retrieved_kv(self, query=None): #将init KV和retrieved KV加载到global_h_k, global_h_v，没global_buffer什么事
        ...
        buffer_len = self.topk * self.block_size + self.topk + self.n_init #token level
        global_h_k = torch.zeros(
                (self.num_units, self.unit_size_kv, buffer_len , self.dim_head),
                dtype = self.global_buffer.dtype, device=self.global_buffer.device
            )
        global_h_v = torch.zeros(
                (self.num_units, self.unit_size_kv, buffer_len , self.dim_head),
                dtype = self.global_buffer.dtype, device=self.global_buffer.device
            )
        ...
        with torch.cuda.stream(GLOBAL_STREAM):
            if self.init_exc:  # init KV were loaded in global_h_k, context KV were offloaded in global_blocks 如果已经加载过init KV，则从global_blocks中reload并copy到global_h_k, global_h_v
                ...
                for u in range(self.num_units):
                    ...
                    for cnt, b_idx in enumerate(self.retrieved_block_indices[u]):
                        # load global_blocks[u][b_idx] onto GPU and make a copy to (global_h_k, global_h_v)
                        st = init_ed + cnt * (self.block_size + 1)
                        ed = st + (self.block_size + 1)
                        self.global_blocks[u][b_idx].load((global_h_k[u, :, st:ed-1, :], global_h_v[u, :, st:ed-1, :])) #reload to GPU and copy it to global_h_k, global_h_v
                        global_h_k[u, :, ed-1, :] = global_h_k[u, :, st:ed-1, :].mean(dim=-2)
                        global_h_v[u, :, ed-1, :] = global_h_v[u, :, st:ed-1, :].mean(dim=-2)
                ...
                for u in range(self.num_units):
                    ...
                    for cnt, b_idx in enumerate(self.retrieved_block_indices[u]):
                        ...
                        if remainder_st >= self.global_remainder[0].size(2):
                            break
                        st = init_ed + cnt * (self.block_size + 1) #cnt
                        ed = st + (self.block_size + 1)
                        global_h_k[u, :, st:ed-1] = self.global_remainder[0][u, :, remainder_st:remainder_ed]
                        global_h_v[u, :, st:ed-1] = self.global_remainder[1][u, :, remainder_st:remainder_ed]
                        global_h_k[u, :, ed-1, :] = self.global_remainder[0][u, :, remainder_st:remainder_ed, :].mean(dim=-2).reshape(self.unit_size, self.dim_head)
                        global_h_v[u, :, ed-1, :] = self.global_remainder[1][u, :, remainder_st:remainder_ed, :].mean(dim=-2).reshape(self.unit_size, self.dim_head)
    ...
    def _calc_block_topk(
        self, global_h_q # (batch_size, num_heads, length, dim_head)
    ):
        ...
        if self.num_global_block <= self.topk:
            ...
            if self.num_global_chunk <= self.topk // min_chunk_size + 1: #最末尾的chunk可能会长度小于8
                ...
                #print('----------------------------------------start------------------------------------')
                #print(f'retrieved_chunk_indices:{retrieved_chunk_indices}')
                #retrieved_chunk_indices = retrieved_chunk_indices.cpu().tolist()
                for u in range(self.num_units):
                    for chunk in retrieved_chunk_indices[u]:
                        ...
                        #assert chunk_ed - chunk_st >= 7,f'chunk_len:{chunk_ed - chunk_st}'
                        hash_map[u].extend(self.chunk2block[u][chunk])
                        #print(f'chunk_st:{chunk_st}, chunk_ed:{chunk_ed},hash_map extend:{self.chunk2block[u][chunk]}')
                        #print(f'chunk_st:{chunk_st}, chunk_ed:{chunk_ed}')
                        #print(f'block_k[u].get_chunk_len:{len(self.block_k[u].get_chunk(chunk_st, chunk_ed))}')
                        #print(f'retrive_k[u]_len_pre:{len(retrive_k[u])}')
                        retrive_k[u].append(self.block_k[u].get_chunk(chunk_st, chunk_ed))
                        # print(f'retrive_k[u]_len_post:{len(retrive_k[u])}')
                        
                #print(f'retrive_k[0]_len:{len(retrive_k[0])}')
                #print(f'hash_map_len:{len(hash_map[0])}')
                #print(f'hash_map:{hash_map[0]}')
                #print('----------------------------------------end------------------------------------')
        ...
        #             ret = [list(range(block_num)) for _ in range(self.num_units)] #全部返回 [[0,1,2,...,block_num-1],[],[],[]]
        #         else:
        ...
        #             global_k = global_k.mean(dim=-2, keepdim=False)  # (batch_size, block_num, dim) 用block的mean作为representative token
        #             logits = torch.matmul(global_k, global_h_q[:, :, None]).squeeze(dim=-1)  #(batch_size, block_num, dim) * (batch_size, dim, 1) = (batch_size, block_num)
        ...
        #     assert self.topk % self.chunk_size == 0 #chunk_size:分组索引 block中的top_k
        #     remainder_size = logits.shape[1] % self.chunk_size #remainder_size:剩余的索引
        #     chunked_logits = logits[:, :logits.shape[1]-remainder_size].reshape(self.num_units, -1, self.chunk_size).mean(dim=-1) #(batch_size, block_num // chunk_size)用chunk的mean作为representative token
        #     if remainder_size > 0:
        #         remainder_logits = logits[:, -remainder_size:].mean(dim=-1, keepdim=True)  # (batch_size, 1) remainder的mean
        #         chunked_logits = torch.cat([chunked_logits, remainder_logits], dim=1) #(batch_size, block_num // chunk_size + 1)
        #     ret = chunked_logits.topk(self.topk//self.chunk_size, dim=1).indices #所有chunk块之中的topk//chunk_size (batch_size, topk//chunk_size)
        #     ret = ret.sort(dim=1)[0][:, :, None]  # (batch_size, topk//chunk_size, 1) indices已排序
        #     ret = ret * self.chunk_size + torch.arange(self.chunk_size, device=ret.device)[None, None, :]  #(batch_size, topk//chunk_size, 1) + (1，1，chunk_size) = (batch_size, topk//chunk_size, chunk_size)
    ...
    def get_global_hidden_and_mask(self, exc_length):
        ...
        # 要一直填到global_remainder_len > self.n_local 之后才会将self.init_exc设置为True,只有True以后才会填global_blocks,所以第一个chunk的大小一定大于self.n_local 
        ...
        self._global_remainder_ed = global_remainder_ed #无论如何，每次都增加exc_length 13
        self._global_remainder_st = global_remainder_st #st前边的是匀给init KV的，如果没有进入if则不变 13
    ...
    def _append(
        ...
        with torch.cuda.stream(GLOBAL_STREAM):
            global_h_q = global_q
            global_h_k, global_h_v = self.get_global_hidden_and_mask(exc_length=global_q.size(-2)) #exc_length or 13
    ...
    def append(
        ...
        if self.init_exc and init_flag:
            ...
            #     f.write(f'st: {self.chunk2block[0][-1][0]},ed: {self.chunk2block[0][-1][-1]},len:{len(self.chunk2block[0][-1])}\n')
