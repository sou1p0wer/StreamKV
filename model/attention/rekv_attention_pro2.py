    ...
    *args, **kwargs
    ...
    def forward(self,query : torch.Tensor,    # (1, 13, D) (1, Nv*196+n_encode, D) (1, seq_len, D)
        ...
        if type(past_key_value) is not ContextManager or past_key_value.to_retrieve:
            ...
            #dynamic_local = max( n_local, past_len - n_init)
            #dynamic_local = past_len
            #dynamic_local = past_len - n_init
            dynamic_local = 25000
        ...
        # if type(past_key_value) is not ContextManager or past_key_value.to_retrieve:
        #     # === 获取 past_k, past_v, 以及合适的 KV cache ===
        #     if type(past_key_value) is ContextManager:  # retrieval
        ...
        #         current_key_value = (torch.cat([past_k, h_k], dim=-2), torch.cat([past_v, h_v], dim=-2))
            
        #     # === 拼接历史与当前key/value，并计算相关长度 ===
        #     h_k = torch.cat([past_k, h_k], dim=-2)
        #     h_v = torch.cat([past_v, h_v], dim=-2)
        #     len_k += past_k.shape[2]
        #     dynamic_window =past_k.shape[2] - n_init  # 可按需加max(1, ...)防止dynamic_window为负
        ...
        #     # === rotary position embedding on init ===
        #     init_h_q = position_bias.apply_rotary_pos_emb_one_angle(
        #         h_q, dynamic_window
        #     )
        ...
        #         h_v = torch.cat([past_v, h_v], dim=-2)
        #         len_k += past_k.shape[2]
        #         dynamic_window = past_k.shape[2] - n_init
        #         current_key_value = (past_k, past_v)
        #         h_q_, h_k_, h_v_ = h_q, h_k, h_v
        #         h_k_ = h_k_[:, :, n_init:, :]
        #         h_v_ = h_v_[:, :, n_init:, :]
        ...
        #         init_h_v = init_h_v[:, :, :n_init, :].contiguous()
            ...
            # print("min =", score.min().item(), "max =", score.max().item(), "mean =", score.mean().item())
            # print("nan?", torch.isnan(score).any().item(), "inf?", torch.isinf(score).any().item())
