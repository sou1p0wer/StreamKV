...
'''
**************************************v4************************************
对应得把n_local填满之后才self.init_exc=True的做法
好处是处理短视频时在一个local下就直接处理了，无需像长视频一样，代码更显鲁棒
坏处是更复杂
**************************************v4************************************
'''
...
class ContextManager:
    ...
    def init(
        ...
        ) #更新（或扩展）cos/sin编码表（缓存表），确保RoPE的cos/sin缓存表大小足够大，能满足当前任务处理的所需最大序列长度和张量形状
    ...
    def _calc_block_topk(
        ...
        logits = None
        '''
        if self.num_global_block <= self.topk:                       global_blocks的数量小于topk，不一定全部返回，因为可能global_reminder中的block数量大于topk
            if not self.init_exc:                                    如果还没有加载init KV， 则global_blocks也未填充，则需从global_remainder中加载block
                if block_num <= self.topk:                           block数量小于topk，则全部返回
                    ret = [list(range(block_num)) for _ in range(self.num_units)]
                else:                                                block数量大于topk，则返回topk
                    logits = torch.matmul(global_k, global_h_q[:, :, None]).squeeze(dim=-1)
            else:                                                    已经加载过init KV， global_blocks已填充，直接加载global_blocks
                ret = [list(range(len(self.global_blocks[0]))) for _ in range(self.num_units)]
        else:                                                        返回global_blocks中的topk
            logits = torch.stack([self.block_k[u].get_cosine_similarity(global_h_q[u]) for u in range(self.num_units)])
        note:两个logits的shape都是(batch_size, block_num)，且计算逻辑一样
        '''
    ...
    def get_global_hidden_and_mask(self, exc_length):
        ...
        if not self.init_exc and global_remainder_len > self.n_local: #global_remainder KV -> init KV
    ...
    def _append(
        ...
        local_h_q, local_h_k = self.position_embedding(local_q, local_k) #旋转位置编码
        local_h_v = local_v
    ...
    def append(
        ...
        # update chunk2block----------------------------
        if not init_flag:
            input_block_num = input_length // self.block_size
            for u in range(self.num_units):
                if self.chunk2block[u] == []:
                    chunk_st = 0
                else:
                    chunk_st = self.chunk2block[u][-1][-1] + 1
                # print(f'chunk_st: {chunk_st}')
                chunk_ed = chunk_st + input_block_num
                chunk2block = list(range(chunk_st,chunk_ed))
                self.chunk2block[u].append(chunk2block)
        ...
        with torch.cuda.stream(GLOBAL_STREAM):
            ...
            ) #只对当前的 global_q 加上“第 self.n_local 个”token的RoPE位置旋转信息，
        ...
        if self.init_exc:
            for u in range(self.num_units):
                while self.num_global_chunk > len(self.chunk_k[u]):
                    num_chunk_k = len(self.chunk_k[u])
                    cur_load_chunk_st = self.chunk2block[u][num_chunk_k][0]
                    cur_load_chunk_ed = self.chunk2block[u][num_chunk_k][-1]
                    if cur_load_chunk_ed + 1 > self.num_global_block:
                        break
                    chunk_k = self.block_k[u].get_mean(cur_load_chunk_st, cur_load_chunk_ed)
                    self.chunk_k[u].append(chunk_k)
                    print(f'chunk_idx:{num_chunk_k},[{cur_load_chunk_st},{cur_load_chunk_ed}]')
        # if self.num_global_chunk!=0:
        #     print(self.chunk2block[0][-1])
        # ----------------------------------------------------------------------
        self.length += input_length
        print(f'len_global_blocks: {self.num_global_block}')
