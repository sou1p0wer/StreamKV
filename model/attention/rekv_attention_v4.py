...
根据append块的大小来填充chunk2block，
好处是处理短视频时在一个local下就直接处理了，无需像长视频一样划分chunk，代码更显鲁棒
坏处是更复杂
**************************************v4************************************
'''
...
def rekv_attention_forward(
    ...
    *args, **kwargs
    ...
    def forward(self, query : torch.Tensor,   #(1, Nv*196, D) (1, 13, D) (1, seq_len, D)
        ...
        # #method 1:attention score 双向注意力是否合理，event后续的帧帮助理解前面的帧
        # h_k = _from_group_kv(h_k, num_heads)
        # attn_score = torch.matmul(h_q, h_k.transpose(-2, -1)) / dim_head ** 0.5 # (batch, num_heads, len_q, len_k)
        # if position_bias is not None: 应该是得加入帧间位置编码 因为我们是一帧一帧encode的
        #     attn_score += position_bias.attn_bias(len_q, len_k)
        # attn_score = attn_score.sum(dim=2) #(batch, num_heads, len_k)
        # attn_score = attn_score.sum(dim=1) #(batch, len_k)
        ...
        if type(past_key_value) is not ContextManager or past_key_value.to_retrieve:
            ...
            len_k += past_k.shape[2] #len_k = init_len + retrieve_len +prompt_len
            ...
            if len_q + n_local < h_k_.size(-2): #len_q + n_local < init_len + retrieve_len + len_k
                h_k_ = h_k_[:, :, h_k_.size(-2) - len_q - n_local:, :]
                h_v_ = h_v_[:, :, h_v_.size(-2) - len_q - n_local:, :]

            local_h_q, local_h_k = position_bias(h_q_, h_k_) #为 h_q_ 和 h_k_ 添加旋转位置编码
            local_h_v = h_v_
            ...
            if len_k > n_local:
                init_h_q = position_bias.apply_rotary_pos_emb_one_angle(
                    h_q, n_local
                ) #对 h_q 应用一次 RoPE 的位置编码，使用的位置是 n_local，使其具备第 n_local 个 token 的相对位置信息。
                init_h_k = h_k
                init_h_v = h_v
