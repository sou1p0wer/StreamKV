...
class VectorTensor:
    ...
    def get_window_cosine_similarity(self, window_size, tensor, chunk_global_sign):
        assert tensor.dim() == 1 and tensor.size(0) == self.hidden_size, f'{tensor.size(0)}, {self.hidden_size}'
        if chunk_global_sign == "True":
        ...
        # if std.item() < 1e-6:
        #     normalized_logits = torch.zeros_like(logits)
        # else:
        ...
        assert logits.dim() == 1 and logits.size(0) == window_size-1 if chunk_global_sign=="True" else window_size ,f'{logits.dim()}, {logits.size(0)},{window_size}'
        return logits
...
class ContextManager:
    def __init__(self, 
        ...
        self.retrieve_size = retrieve_size # number of blocks to retrieve
    ...
    def calculate_score(self, video_len, prompt):
        ...
        logits = torch.stack([self.block_k[u].get_window_cosine_similarity(num_frames,prompt[u],self.chunk_global_sign) for u in range(self.num_units)]) #(batch_size, num_frames)
        self.layer_compression_score = logits

    def compress_kv(self, video_len, prompt = None):
        num_frames = video_len // self.block_size

        if self.compress_mode == 'online': #online
            logits = self.layer_compression_score #(batch_size, num_frames)
            assert logits.shape == (self.num_units, num_frames - 1) if self.chunk_global_sign == "True" else (self.num_units, num_frames)
            reserved_num = self.reserved_num
        else: #offline or base
            prompt = prompt.mean(dim=2, keepdim=False)
            assert prompt.shape == (self.num_units, self.unit_size, self.dim_head)
            prompt = prompt.reshape(self.num_units, self.dim_head * self.unit_size)
            logits = torch.stack([self.block_k[u].get_window_cosine_similarity(num_frames,prompt[u],self.chunk_global_sign) for u in range(self.num_units)]) #(batch_size, num_frames) or (batch_size, num_frames - 1)
            #assert logits.shape == (self.num_units, num_frames)
            reserved_num = self.reserved_num
        ...
        if self.chunk_global_sign == "True":
            ...
            kept_indices = torch.cat([kept_indices, global_indices], dim=1) # (batch_size, num_frames_after_compressed + 1)
        ...
        #print(f'chunk2block: {self.chunk2block[0][-1]}')
    ...
    def get_retrieved_kv(self,query=None): #将init KV和retrieved KV加载到global_h_k, global_h_v，没global_buffer什么事
        ...
        assert global_h_k.size(-2) <= self.n_init + self.max_retrive_block * self.block_size
    ...
    def _calc_block_topk(
        ...
        if self.retrieve_local == 'False': #不强制检索最近的几个block
            ...
            if self.num_global_block <= retrieval_num:
                ...
                ret = ret.cpu().tolist()
        else: #强制检索最近的几个block 目前这种做法在online的算分时也强制后面几个block
            logits = torch.stack([self.block_k[u].get_cosine_similarity(global_h_q[u]) for u in range(self.num_units)]) #(batch_size, num_global_block)
            if self.retrieval_mode == 'online' and self.retrieval_prefill:
                self.layer_retrieval_score = logits
            ...
            for u in range(self.num_units):
                if self.num_global_block <= retrieval_num:
                    ...
                    if retrieval_num >= self.retrieve_local_size:
                        ...
                        new_logits = logits[u][:-self.retrieve_local_size]  # (num_global_block-retrieve_local_size,)
    ...
    def _append_global(
        ...
        if self.init_exc: #已经填充了init KV
            ...
            while global_remainder_len > 0:
                ...
                global_block_k = self.global_remainder[0][:, :, global_remainder_st:global_remainder_st + self.block_size, :] #(batch_size, num_heads_kv, length, dim_head)
    ...
    def append(
        ...
        if not compression_flag or self.encode_mode == 'dense':
